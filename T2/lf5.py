...
def weighted_average(ms_master, derivs_master, neighbours, z):
    ...
    elif len(ms_master[0])>len(ms_master[1]):
...
index = pd.MultiIndex.from_product((df["Kmag"], df["MH"]), names=['Kmag', 'MH'])
df2 = pd.DataFrame(index=index)
df2['mass'] = df["masses"]
interp = DFInterpolator(df2)
...
def thetainv(M, z, typ, show_plt):
    ...
    if z < metallicities.min():
        z = metallicities.min()
    elif z > metallicities.max():
        z = metallicities.max()
    z_neighbours = find_neighbours(data[:,0], z)
    ms_master = []
    derivs_master = []
    for nearestz in z_neighbours:
    ...
    ms, derivs = weighted_average(ms_master, derivs_master, z_neighbours, z)
    return ms, derivs
